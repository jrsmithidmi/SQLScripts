...
/*
EXEC msdb.dbo.sp_delete_operator @name=N'SilvervineDBA'
GO


EXEC msdb.dbo.sp_add_operator @name=N'SilvervineDBA', 
		@enabled=1, 
...
DECLARE @OperatorName sysname = N'SilvervineDBA';
...
DECLARE @Sev18AlertName sysname = @ServerName + N' Alert - Sev 18 Error: Nonfatal Internal Error Detected';
...
DECLARE @Error15327AlertName sysname = @ServerName + N' Alert - Error 15327: Database Offline';
...
-- Error 15327 Alert added on 6/2/2020

-- Error 15327: Logical consistency-based I/O error
-- How to troubleshoot Msg 824 in SQL Server
-- http://support.microsoft.com/kb/2015756
IF NOT EXISTS (SELECT name FROM msdb.dbo.sysalerts WHERE name = @Error15327AlertName)
	EXEC msdb.dbo.sp_add_alert @name = @Error15327AlertName, 
				  @message_id = 15327, @severity = 0, @enabled = 1, 
				  @delay_between_responses = 900, @include_event_description_in = 1, 
				  @category_name = @CategoryName, 
				  @job_id  = N'00000000-0000-0000-0000-000000000000';
...
IF NOT EXISTS(SELECT *
              ...
              INNER JOIN dbo.sysnotifications AS sn
              ON sa.id = sn.alert_id
              WHERE sa.name = @Error15327AlertName)
	BEGIN
		EXEC msdb.dbo.sp_add_notification @alert_name = @Error15327AlertName, @operator_name = @OperatorName, @notification_method = 1;
	END
